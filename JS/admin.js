...
<td>
    ...
    // CANCELLED
    const ovCancel = document.getElementById("ov_cancelledBookings");

    if (ovCancel) ovCancel.textContent = cancelled;
    ...
    let maxCap =
        parseInt(localStorage.getItem("dailyCapacity")) || 100;
...
function renderCalendar() {
    ...
    orders.forEach(order => {
        ...
        if (order.datetime) {
            ...
            const dateKey =
                `${orderDate.getFullYear()}-${orderDate.getMonth()}-${orderDate.getDate()}`;
    ...
    for (
        ...
        if (i > paddingDays) {
            ...
            const currentSquareKey =
                `${year}-${month}-${dayNumber}`;

            const daysBookings =
                ordersByDate[currentSquareKey];
            ...
            if (
                ...
                indicator.style.color = "#bc6c25";
                indicator.style.fontWeight = "600";
                indicator.style.marginTop = "6px";

                indicator.textContent =
                    `● ${daysBookings.length} Booking${daysBookings.length > 1 ? "s" : ""}`;

                daySquare.appendChild(indicator);

                daySquare.style.border =
                    "2px solid rgba(188,108,37,0.3)";
                ...
                daySquare.addEventListener(
                    ...
                    () => {
                        ...
                        let details =
                            `📅 Reservations for ${monthDisplay.textContent.split(" ")[0]} ${dayNumber}, ${year}:\n\n`;
                        ...
                        daysBookings.forEach(
                            (b, index) => {
                                ...
                                details +=
                                    `${index + 1}. ${b.name} @ ${timeStr}\n`;

                                details +=
                                    `   Contact: ${b.contact}\n`;

                                details +=
                                    `   Service: ${b.service} (${b.guests} guests)\n`;
